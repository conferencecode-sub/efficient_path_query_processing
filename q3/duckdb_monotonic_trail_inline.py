...
class DuckDBMonotonicTrailDB:
    """DuckDB implementation using monotonic trail"""
    ...
    def run_with_pure_monotonic_trail_udf(self, 
                                    min_length,
                                     max_length, starter_node) -> Tuple[int, float]:
        """
        Run DuckDB query using the combined monotonic trail inline
        """
...
def main():
    parser = argparse.ArgumentParser(description='DuckDB Monotonic Trail UDF')
    ...
    duckdbs = DuckDBMonotonicTrailDB()
    
    print("-"*50)
    print("Reading data from:", args.edges)
    
    # Load data
    duckdbs.load_data(args.nodes, args.edges, args.nfanodes, args.nfa, True)
    
    starter_node = 1  # change if needed
    n = 10 # max path length to test
    for i in range(2, n+1):
        duckdbs.run_with_pure_monotonic_trail_udf(2, i, starter_node)
