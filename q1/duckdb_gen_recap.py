...
class DuckDBGeneralReCAPQuery:
    ...
    def run_gen_recap_query(self, min_length: int, max_length: int, starter_node: int) -> Tuple[int, float]:
        c = self.conn
...
def main():
    parser = argparse.ArgumentParser(description='DuckDB GenReCAP Query')
    ...
    duckdbs = DuckDBGeneralReCAPQuery()
    ...
    for max_len in range(2,11):
        duckdbs.run_gen_recap_query(2, max_len, starter_node)
