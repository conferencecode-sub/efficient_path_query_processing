...
class InlineReCAPQuery:
    """ReCAP implementation using combined property and regex constraints with UDFs."""
    ...
    def run_gen_recap_inline(self, min_length: int, max_length: int, starter_node: int) -> Tuple[int, float]:
        c = self.conn
    ...
    -- Base case
            SELECT
                {starter_node} as v, 
                0 as state,
                NULL::DOUBLE as max_risk,
                NULL::DOUBLE as min_risk,
                NULL::DOUBLE as last_risk,
                CAST(-99999 AS INT64) as last_time,
            ...
            SELECT 
                ...
                LEAST(p.min_risk, t.risk_score) as max_risk,
                t.risk_score as last_risk,
                t.timestamp_ms as last_time,
                t.location_region as region,
            ...
            WHERE p.path_length < {max_length} -- change
                AND (t.location_region = region OR region IS NULL)
                AND t.timestamp_ms > last_time 
                AND (CASE WHEN p.state = 0 AND n.to_state = 1 THEN (GREATEST(p.max_risk, t.risk_score) - LEAST(p.min_risk, t.risk_score) <= 20)  OR (max_risk IS NULL)
                        WHEN p.state = 1 AND n.to_state = 1 THEN (GREATEST(p.max_risk, t.risk_score) - LEAST(p.min_risk, t.risk_score) <= 20)  OR (max_risk IS NULL) 
                        WHEN p.state = 1 AND n.to_state = 2 THEN last_risk >= 40
...
def main():
    parser = argparse.ArgumentParser(description='ReCAP GenReCAP Query (Inline Version)')
    ...
    recap = InlineReCAPQuery()

    print("-" * 50)
    print("Reading data from:", args.nodes)
    print("Running Color+Trail ReCAP", "with index")

    recap.load_data(args.nodes, args.edges, args.nfanodes, args.nfa, True)

    starter_node =  383 # change if needed
    print("-" * 50)
    for max_len in range(2,11):
        recap.run_gen_recap_inline(2, max_len, starter_node)
