#!/usr/bin/env bash
# Runs Q4 (max-min trail) on Memgraph (experiments/SOA-GDBMS/
...
# memgraph_run.py (same fix as neo4j_run.py/kuzu_run.py -- see README.md).
...
python3 "$SOA_DIR/memgraph_run.py" \
  ...
  --fresh-db \
  --csv "results/memgraph_q4.csv"

echo "Completed -- results in results/memgraph_q4.csv"
