"""Sweeps Q3 (monotonic trail) over the old prototype's three ReCAP/q3
variants -- DuckDB baseline, ReCAP-inline, ReCAP-UDF -- at max_length in
{2,3,4} (min_length=2 fixed, starter_node=9, same as q2_length_sweep, over
the same generic 100-node dataset).

Same isolation approach as q1/q2_length_sweep's own run_old_prototype.py
...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "..", "..", "ReCAP", "q3"))
...
LENGTHS = (2, 3, 4)
WARMUP, RUNS = 1, 3
CSV_PATH = os.path.join(os.path.dirname(__file__), "results", "old_prototype_q3.csv")
...
def _class_and_method(engine: str):
    if engine == "duckdb-baseline":
        from duckdb_monotonic_trail_inline import DuckDBMonotonicTrailDB
        return DuckDBMonotonicTrailDB, "run_with_pure_monotonic_trail_udf"
    if engine == "recap-inline":
        from recap_monotonic_trail_inline import RecapInlineMonotonicTrailDB
        return RecapInlineMonotonicTrailDB, "run_with_inline_monotonic_trail"
    if engine == "recap-udf":
        from recap_monotonic_trail_UDF import PureRecapMonotonicTrailDB
        return PureRecapMonotonicTrailDB, "run_with_pure_monotonic_trail_udf"
