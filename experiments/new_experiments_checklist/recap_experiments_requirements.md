...
> These four already discharge the *headline* speedup and the root-cause story. The new experiments below target the specific reviewer doubts they do **not** yet answer.

---

## B. New experiments required by reviewers

### E4 — Isolation: automata-exploration vs property early-filtering  **[Tier 1]**
**Reviewer:** R3.O1 (required change, insisted on), and meta-crux (3). Also underwrites flow-Q8.
**Question it answers:** how much of the speedup is the NFA-as-a-join (regex exploration) versus the property early-filtering? R3 notes prior work ([13]) shows the NFA approach *alone* already helps, and wants that separated from ReCAP's property pruning.
...
**Sweep the two axes R3 named:** (a) regex selectivity — run on a non-trivial pattern at several selectivities; (b) path returned vs not returned (endpoints only vs full edge-id sequence), since returning paths changes the pruning/storage story.
...
**Reviewer:** R4.O1 (R4's single ranked point; "if ReCAP shows better performance, that changes the game"). Underwrites flow-Q6/Q7.
**Question it answers:** is ReCAP just a "veneer" over hand-pushing the constraint into the `RECURSIVE` clause?
**Design.** Same queries/graphs, three plans:
1. **Handcrafted** — constraints inlined by hand in the recursive `WHERE` (the plan R4 says the engine already gets).
...
**Note (correctness companion, not an experiment):** the correctness theorem (two local obligations vs undecidable recursive-query equivalence) is the *non-performance* half of the R4 answer — it lives in the body/appendix, not here, but E5 is what R4 explicitly asked to see.

### E6 — End-to-end on a realistic workload (FinBench / LDBC)  **[Tier 1]**
**Reviewer:** R5.O1 (R5's ranked point). Underwrites flow-Q11.
...
**Reviewer:** R3.O2 (graph size), R3.O3 (memory; and "only 4 queries" concern).
**Design (two parts).**
...
- **Memory:** report peak memory for ReCAP vs a competitor, specifically in the **many-viable-paths regime** R3 worried about (full-path/trail state stored). Include at least one query where early filtering does *not* collapse the state (so storage is genuinely stressed), per R3.O3.
**Metric.** Runtime + peak RSS (via DuckDB profiling / `/usr/bin/time`). 

### E8 — `Q2` clarification (piggybacking without early filtering)  **[Tier 2]**
**Reviewer:** R5.M9 (Fig. 8/9 labeling: is "ReCAP" = Optimized? how does Standard compare to competitors on Q2?).
...
## C. Regex realism (addresses R3's "non-trivial pattern" and strengthens E4/E5)

Reviewers want the regex side to be non-trivial and not cherry-picked. Use **real query-log studies** to justify the *shape* of the label patterns (Kleene-star/plus, unions, concatenations) that make constraints list-based:
...
**Use (scope-limited):** cite these to argue that property-path patterns with transitive closure (the patterns that *force* list-based constraints, per §Problem) are common in practice, and pick the regex complexity/selectivity levels in E4/E5 to match the distribution these logs report (e.g., prevalence of `*`/`+`, typical alternation width). **Do not** claim to replay the logs — they are RPQ/label workloads without ReCAP's property constraints; they justify the *regex* dimension only. This directly answers R3.O1's "hopefully on a non-trivial pattern" and R3's general "only 4 queries" unease by grounding the pattern space in measured reality.

---

## D. What each reviewer sees answered

| Reviewer point | Experiment(s) |
|---|---|
| R3.O1 (isolate automata vs pruning; non-trivial regex; path returned or not) | **E4** (+ regex realism, §C) |
| R3.O2 (modest graph sizes) | **E7 scale** (Datagen-7.7) |
| R3.O3 (path return + memory; only 4 queries) | **E7 memory** (+ E4 path-returned axis) |
| R4.O1 (why not hand-write into RECURSIVE / veneer) | **E5** (perf) + correctness theorem (body, not an experiment) |
| R4.O2 (integration with richer plans) | **E5 config 3** (split/reverse) |
| R5.O1 (real workloads / overall performance) | **E6** (FinBench/LDBC end-to-end) |
| R5.M9 (Fig labeling / Q2 Standard vs competitors) | **E8** (presentation) |
...
- **Tier 1 (must land in body):** E4 (R3's insisted change), E5 (R4's crux), E6 (R5's ranked point). These three map 1:1 onto the three Major-revision reviewers' top asks. If only these fit, the revision still answers every ranked concern.
