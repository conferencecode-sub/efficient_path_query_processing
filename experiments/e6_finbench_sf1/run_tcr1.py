"""E6/SF1 / TCR1: sweeps ReCAP (Stage F, optimized) over increasing
...
(SF0.1), just pointed at SF1 data with a start vertex re-derived for SF1's
...
# Re-derived for SF1 via the same methodology as SF0.1 ("highest transfer
# out-degree"): `SELECT src, COUNT(*) FROM edges WHERE label='transfer'
# GROUP BY src ORDER BY COUNT(*) DESC` against SF1's own loaded data ->
# out-degree 945 (tied with one other vertex at the same degree).
START_VERTEX = 300615275126988483
