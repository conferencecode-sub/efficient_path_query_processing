"""E6/SF10 / TCR5: sweeps ReCAP (Stage F, optimized) over increasing
...
(SF0.1), start vertex re-derived for SF10's own graph.
"""
...
# Re-derived for SF10 via the same methodology as SF0.1/SF1 ("person with
# most transfer-reachable own-accounts at length 2"): joining own->transfer
# and grouping by the person, against SF10's own loaded data -> 4,628
# reachable. Verified non-zero (72,809 at length_bound=4) via
...
MIN_LENGTH = 2  # TCR5's accepting state (own hop + at least one transfer) is
                 # only reachable at path_length >= 2 anyway -- floor here so
                 # intermediate_paths matches the competitor scripts' own
                 # candidate-only queries apples-to-apples.
