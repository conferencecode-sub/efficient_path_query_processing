...
class Neo4jBenchmark:
    ...
    def run_q3_monotonic_trail_query(self, min_len: int, max_len: int, warmup: int, runs: int, starter_node: int):
        ...
        query = f"""
            MATCH p = (s WHERE s.id = {starter_node})-[r *{min_len}..{max_len}]-> (t)
    ...
    def run_q4_max_min(self, min_len: int, max_len: int, warmup: int, runs: int, starter_node: int):
        query = f"""
            MATCH p = (s WHERE s.id = {starter_node})-[*{min_len}..{max_len}]->(t)
    ...
    def run_q1_regex(self, min_len: int, max_len: int, warmup: int, runs: int, starter_node: int):
        query = f"""
                MATCH p = (s WHERE s.id = {starter_node})
