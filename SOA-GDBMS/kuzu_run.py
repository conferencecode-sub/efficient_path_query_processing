...
class KuzuBenchmark:
    ...
    def run_relaxed_query(self, min_len: int, max_len: int, warmup: int, runs: int):
        query = f"""
            MATCH (start:Node {{id: 359281}})-[e:Edge*{min_len}..{max_len} (r, _ | WHERE r.weight > 0 AND r.color IN ['red', 'blue'])]->()
            RETURN count(*)
        """
        return self.run_query(query, f"Relaxed Paths [{min_len},{max_len}]", warmup, runs)
    
    def run_gen_recap_q(self, min_len: int, max_len: int, warmup: int, runs: int):
        query = f"""
            MATCH path = (start:Node {{id: 383}})-[normal:transfer|sale|purchase *1..{max_len}]->
                (mid:Node)
    ...
    def run_q1_any_colors_query(self, min_len: int, max_len: int, warmup: int, runs: int):
        query = f"""
            MATCH path = (start:Node {{label: 'Start'}})-[e:Edge*{min_len}..{max_len}]->()
    ...
    def run_q2_monotonic_query(self, min_len: int, max_len: int, warmup: int, runs: int):
        query = f"""
            MATCH path = (start:Node {{id: 6113}})-[*{min_len}..{max_len}]->(t)
            WITH path as path,
            list_reduce(
    ...
    def run_q3_max_min_query(self, min_len: int, max_len: int, warmup: int, runs: int):
        query = f"""
            MATCH path = (start:Node {{id: 320104}})-[*{min_len}..{max_len}]->(t)
            WITH path as path,
            list_reduce(
                    properties(rels(path), 'weight'),
                            (acc, w) ->
    ...
    def run_q5_altredblue_query(self, min_len: int, max_len: int, warmup: int, runs: int):
        query = f"""
            MATCH (start:Node {{id: 359281}})-[e:Edge*{min_len}..{max_len} (r, _ | WHERE r.weight > 0 AND r.color IN ['red', 'blue'])]->()
            WITH path as path,
            list_reduce(
            ...
            WHERE total_weight <= 1 AND is_trail(path) 
            RETURN COUNT(*);
        """
        return self.run_query(query, f"Monotonic growing paths [{min_len}..{max_len}]", warmup, runs)
...
def main():
    ...
    for length in range(MIN_LENGTH, MAX_LENGTH + 1):
        results.append(bench.run_gen_recap_q(MIN_LENGTH, length, WARMUP_RUNS, TIMED_RUNS))
