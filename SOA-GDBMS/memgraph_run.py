...
class MemgraphBenchmark:
    ...
    def run_q1_gen_recap_query(self, min_len: int, max_len: int, warmup: int, runs: int, starter_node: int):
        query = f"""
            MATCH p = (start:Node {{id: {starter_node}}})
    ...
    def run_q4_max_min(self, min_len: int, max_len: int, warmup: int, runs: int, starter_node: int):
        query = f"""
            MATCH p = (start:Node {{id: {starter_node}}})-[e:Edge*{min_len}..{max_len}]->(end)
    ...
    def run_q3_monotonic_trail_query(self, min_len: int, max_len: int, warmup: int, runs: int, starter_node: int):
        # Memgraph supports similar list comprehensions to Neo4j
        query = f"""
            MATCH path = (start:Node {{id: {starter_node}}})-[e:Edge*{min_len}..{max_len}]->(end)
    ...
    def run_q2_same_color_trail_query(self, min_len: int, max_len: int, warmup: int, runs: int, starter_node: int):
        query = f"""
            MATCH path = (start:Node {{id: {starter_node}}})-[e:Edge*{min_len}..{max_len}]->(end)
...
def main():
    ...
    try:
        ...
        for length in range(MIN_LENGTH, MAX_LENGTH + 1):
            results.append(bench.run_gen_recap_query(MIN_LENGTH, length, WARMUP_RUNS, TIMED_RUNS, starter_node))
